...
import fuzziersort from '../dist/fuzziersort.js';
...
export let testdata_prepared = {}; let testdata_obj = {}
{
  for(const key of Object.keys(testdata)) {
    ...
    for(let i = testdata[key].length-1; i>=0; i-=1) {
      testdata_prepared[key][i] = fuzziersort.prepare(testdata[key][i])
    }
  }
  for(const key of Object.keys(testdata)) {
...
async function tests() {
  ...
  { // urls with garbage ids in them match everything too well
    ...
    var tmp = fuzziersort.go('zom', targets)
  ...
  { // Exponential backtracking https://github.com/farzher/fuzzysort/issues/80
    ...
    var tmp = fuzziersort.single('aaaaaaaaaaab', 'a a a a a a a a a a a a a a a a a a a a a a a a a a a a a a a xb')
  ...
  { // spaces destroy the score even when it's an exact substring https://github.com/farzher/fuzzysort/issues/99
    assert(fuzziersort.single('this is exactly the same search and target', 'this is exactly the same search and target').score == 1)
  ...
  { // object cache overwriting your results
    var result1 = fuzziersort.single('a', 'pants')
    var result2 = fuzziersort.single('s', 'pants')
    assert(result1.score != result2.score, 'different scores single')

    var result1 = fuzziersort.go('a', ['pants'])[0]
    var result2 = fuzziersort.go('s', ['pants'])[0]
    assert(result1.score != result2.score, 'different scores go')

    var result1 = fuzziersort.go('a', [{title: 'pants'}], {key: 'title'})[0]
    var result2 = fuzziersort.go('s', [{title: 'pants'}], {key: 'title'})[0]
    assert(result1.score != result2.score, 'different scores key')

    var result1 = fuzziersort.go('a', [{title: 'pants'}], {keys: ['title']})[0]
    var result2 = fuzziersort.go('s', [{title: 'pants'}], {keys: ['title']})[0]
    assert(result1.score != result2.score, 'different scores keys')

    var result1 = fuzziersort.go('a', [{title: 'pants'}], {keys: ['title']})[0][0]
    var result2 = fuzziersort.go('s', [{title: 'pants'}], {keys: ['title']})[0][0]
    assert(result1.score != result2.score, 'different scores keys result')
  }

  { // numbers should work
    var result = fuzziersort.single(5, 256)
  ...
  { // Cannot read properties of undefined (reading '_score') https://github.com/farzher/fuzzysort/issues/130
    ...
    var result = fuzziersort.go('a'  , [{pants:'what'}, {apple:'what'}, {x:'a', '1':'idk'}], {keys:['pants', 'x', 'y', 'z', '']})
    var result = fuzziersort.go('a a', [{pants:'what'}, {apple:'what'}, {x:'a', '1':'idk'}], {keys:['pants', 'x', 'y', 'z', '']})
  }
  ...
  { // options.all
    var tmp = fuzziersort.go('', ['pants', 's', ''], {all: true})
    assert(tmp.length==3, 'all 1')

    var tmp = fuzziersort.go('pants', ['pants', 's', ''], {all: true})
    assert(tmp.length==1, 'all 2')

    var tmp = fuzziersort.go('', ['pants'])
    assert(tmp.length==0, 'all 3')

    var tmp = fuzziersort.go('', [{a:'pants', b:'noodles'}, {a:'suit', b:'tie'}], {keys:['a', 'b'], all: true})
    assert(tmp[1][1].target === 'tie', 'all 5')

    var targets = [{a:1}, {a:2}]
    var tmp = fuzziersort.go('', targets, {key:'a', all: true})
    assert(tmp[0].obj === targets[0], 'options.all with key https://github.com/farzher/fuzzysort/issues/134')

    var tmp = fuzziersort.go('', targets, {keys:['a'], all: true})
  ...
  { // weird characters require strict match to turn into substring
    ...
    var results = fuzziersort.go('Ääsma', targets)
    assert(results[0] && results[0].highlight('*', '*') === "Änni *ÄÄsma*", "Änni *ÄÄsma*")
    var results = fuzziersort.go('aasma', targets)
    assert(results[0] && results[0].highlight('*', '*') === "Änni *ÄÄsma*", "Änni *ÄÄsma*")
  }

  { // normalize diacritics / accents / ligatures
    var targets = ['Café Society']
    var results = fuzziersort.go('cafe', targets)
    assert(results[0].target == 'Café Society')

    // make sure ジ highlights correctly
    assert(fuzziersort.go('ジ', ['ファイナルファンタジーXIV スターターパック'])[0].highlight() == 'ファイナルファンタ<b>ジ</b>ーXIV スターターパック', 'ジ')

    // make sure this doesn't infinite loop
    assert(fuzziersort.go('w', ['Rich JavaScript Applications – the Seven Frameworks (Throne of JS, 2012) - Steve Sanderson’s blog - As seen on YouTube™']).length != 0)
  }

  { // find good substrings, don't just use theh first one
    var result = fuzziersort.single('cat', 'basecategory cat')
    assert(result.highlight('*', '*') === 'basecategory *cat*', 'basecategory cat')
  }

  { // make sure indexes are correct for search with spaces
    var tmp = fuzziersort.single('pants on the ground', 'electron-uwp-background: Sample explaining how to use')
  ...
  { // straw berry should match against strawberry well
    var tmp = fuzziersort.single('straw berry', 'strawberry')
    assert(tmp.score > -50, 'straw berry')
  }


  { // random junks tests, idk
    var tmp = fuzziersort.go('cman', testdata_prepared.ue4_files).slice(0, 2).map(r=>r.target)
    assert(tmp.includes('CheatManager.h') && tmp.includes('CrowdManager.h'), 'cman', tmp[0])

    var tmp = fuzziersort.go('a', ['ba', 'bA', 'a', 'bA', 'xx', 'ba'])
    ...
    assert(fuzziersort.go('a', ['a', 'a']).length===2, 'length')
    assert(fuzziersort.go('a', ['a', 'a'], {limit: 1}).length===1, 'length')
    ...
    // key
    var tmp = fuzziersort.go('str', tmpObjs, {key: 's.s'})[0]
    assert(tmp.score===1, 'goKey s.s')
    var tmp = fuzziersort.go('obj', tmpObjs, {key: 'arr.0.o'})[0]
    assert(tmp.score===1, 'goKey arr.0.o')
    var tmp = fuzziersort.go('str', tmpObjs, {key: 'arr.0.o'})[0]
    assert(tmp===undefined, 'goKey')
    var tmp = fuzziersort.go('obj', tmpObjs, {key: ['arr', '0', 'o']})[0]
    assert(tmp.score===1, 'goKey arr.0.o')

    // keys
    var tmp = fuzziersort.go('str', tmpObjs, {keys: ['s.s']})[0]
    assert(tmp.score===1, 'goKeys s.s')
    var tmp = fuzziersort.go('obj', tmpObjs, {keys: ['arr.0.o']})[0]
    assert(tmp.score===1, 'goKeys arr.0.o')
    var tmp = fuzziersort.go('str', tmpObjs, {keys: ['arr.0.o']})[0]
    assert(tmp===undefined, 'goKeys')
    var tmp = fuzziersort.go('obj', tmpObjs, {keys: [ ['arr', '0', 'o'] ]})[0]
    assert(tmp.score===1, 'goKeys arr.0.o')
    var tmp = fuzziersort.go('obj', tmpObjs, {keys: [ 's.s', 'arr.0.o' ]})[0]
    ...
    var results = fuzziersort.go('typography', targets, {key: 'name'})
    assert(results[0].obj.version != results[1].obj.version, 'key same object bug')
    var results = fuzziersort.go('typography', targets, {keys: ['name']})
    ...
    var s = Symbol()
    var results = fuzziersort.go('va', ['value', ''].map(v => ({target: v, [s]: fuzziersort.prepare(v)})), {key: s})
    assert(results.length==1)
    var results = fuzziersort.go('va', ['value', ''].map(v => ({target: v, [s]: fuzziersort.prepare(v)})), {keys: [s]})
    assert(results.length==1)

    // make sure we don't deoptimize when run with weird values
    fuzziersort.single('sup', fuzziersort.prepare())
    fuzziersort.go('', ['empty search test'])
    fuzziersort.go('empty target test', [''])
    fuzziersort.go('', [''])

    var results = fuzziersort.go('farzher', [{yes:'farzher', no:'no'}], {keys:['yes', 'no']})
    assert(!!results[0].obj)

    var results = fuzziersort.go('doesnotexist', [])
    assert(results.length === 0 && results.total === 0, 'no results')

    var results = fuzziersort.go('ryan berry', [
    ...
    fuzziersort.go('xxxxxxx', [{a:'nomatch x'}], {key: 'a'}) // running key when there's no match but bitflags matches to make sure no error

    var results = fuzziersort.go('a', [{a:'a 2222'}, {a:'a 1'}], {key: 'a', limit:1})
    assert(results.length === 1 && results.total === 2, 'limit & total key')
    assert(results[0].obj.a === 'a 1', 'limit sorting key')

    var results = fuzziersort.go('a', ['a 2222', 'a 1'], {limit:1})
    assert(results.length === 1 && results.total === 2, 'limit & total nokey')
    assert(results[0].target === 'a 1', 'limit sorting nokey')

    var results = fuzziersort.go('sup', [{a:'the wordsup is in here but not well'}], {key:'a', threshold:.5})
    assert(results[0] === undefined, 'threshold low')
    var results = fuzziersort.go('sup', [{a:'the wordsup is in here but not well'}], {key:'a', threshold:.1})
    assert(results[0] !== undefined, 'threshold high')

    assert(fuzziersort.single('soup time ifs', 'time for soup').highlight((m, i) => `*${m}(${i})*`).join('') === '*time(0)* *f(1)*or *soup(2)*', 'highlight callback')

    // indexes getter and setter
    fuzziersort.single('pants ground', 'pants on the ground')
    var result = fuzziersort.single('p', 'pants on the ground')
    ...
    var result = fuzziersort.single('t o g p', 'pants on the ground')
    assert(result.indexes.length === 4 && result.indexes[0] === 0 && result.indexes[1] === 6 && result.indexes[2] === 9 && result.indexes[3] === 13, 'indexes')

    assert(fuzziersort.single('ui', 'build ui').score === fuzziersort.single('ui', 'b__ld ui').score, 'we should find the better substring and scores here should be the same')

    var results = fuzziersort.go('zzzzzzzzzzzzz', [{a:'no thanksz'}], {keys:['a', 'a']})
    assert(results.length === 0, 'no match on keys')

    fuzziersort.go('pants', [['pants on the ground']], {key: obj => obj[0]})
    fuzziersort.go('pants', [['pants on the ground']], {keys: [obj => obj[0], obj => obj[0]]})
  }
}
...
function testSorting(search, ...targets) {
  var results = fuzziersort.go(search, targets)
...
function test(target, ...searches) {
  ...
  for (var i = 0; i < searches.length; i++) {
    ...
    const result = fuzziersort.single(searches[i], target)
...
function testStrict(target, ...searches) {
  for(const search of searches) {
    const result = fuzziersort.single(search, target)
...
async function benchmarks() {
  ...
  bench('spaces & keys', function() {
    fuzziersort.go('github react',      testdata_obj.urls_and_titles, {keys: ['str.title', 'str.url'], limit:100})
    fuzziersort.go('zom query',         testdata_obj.urls_and_titles, {keys: ['str.title', 'str.url'], limit:100})
    fuzziersort.go('image video gif',   testdata_obj.urls_and_titles, {keys: ['str.title', 'str.url'], limit:100})
  }, config)

  const results = fuzziersort.go('e', testdata_prepared.ue4_files, {limit:100})
  ...
  bench('tricky', function() {
    fuzziersort.single('prrun', 'C:/users/farzher/dropbox/someotherfolder/pocket rumble refactor/Run.bat')
  }, config)
  bench('tricky space', function() {
    fuzziersort.single('pr run', 'C:/users/farzher/dropbox/someotherfolder/pocket rumble refactor/Run.bat')
  }, config)
  bench('tricky lot of space', function() {
    fuzziersort.single('pr run      e     e e e        e      e e ee         e     e', 'C:/users/farzher/dropbox/someotherfolder/pocket rumble refactor/Run.bat')
  }, config)

  bench('go prepared spaces', function() {
    fuzziersort.go('n n  n    n     e', testdata_prepared.ue4_files, {limit:100})
    fuzziersort.go(' e ',               testdata_prepared.ue4_files, {limit:100})
    fuzziersort.go('m render .h',       testdata_prepared.ue4_files, {limit:100})
  }, config)
  bench('go key spaces', function() {
    fuzziersort.go('n n  n    n     e', testdata_obj.ue4_files, {key: 'str', limit:100})
    fuzziersort.go(' e ',               testdata_obj.ue4_files, {key: 'str', limit:100})
    fuzziersort.go('m render .h',       testdata_obj.ue4_files, {key: 'str', limit:100})
  }, config)

  bench('go prepared', function() {
    fuzziersort.go('nnnne', testdata_prepared.ue4_files, {limit:100})
    fuzziersort.go('e', testdata_prepared.ue4_files, {limit:100})
    fuzziersort.go('mrender.h', testdata_prepared.ue4_files, {limit:100})
  }, config)
  bench('go prepared nolimit', function() {
    fuzziersort.go('nnnne', testdata_prepared.ue4_files)
    fuzziersort.go('e', testdata_prepared.ue4_files)
    fuzziersort.go('mrender.h', testdata_prepared.ue4_files)
  }, config)
  bench('go prepared key', function() {
    fuzziersort.go('nnnne', testdata_obj.ue4_files, {key: 'prepared', limit:100})
    fuzziersort.go('e', testdata_obj.ue4_files, {key: 'prepared', limit:100})
    fuzziersort.go('mrender.h', testdata_obj.ue4_files, {key: 'prepared', limit:100})
  }, config)
  bench('go key', function() {
    fuzziersort.go('nnnne', testdata_obj.ue4_files, {key: 'str', limit:100})
    fuzziersort.go('e', testdata_obj.ue4_files, {key: 'str', limit:100})
    fuzziersort.go('mrender.h', testdata_obj.ue4_files, {key: 'str', limit:100})
  }, config)
  bench('go keys', function() {
    fuzziersort.go('nnnne', testdata_obj.ue4_files, {keys: ['str'], limit:100})
    fuzziersort.go('e', testdata_obj.ue4_files, {keys: ['str'], limit:100})
    fuzziersort.go('mrender.h', testdata_obj.ue4_files, {keys: ['str'], limit:100})
  }, config)
  bench('go str', function() {
    fuzziersort.go('nnnne', testdata.ue4_files, {limit:100})
    fuzziersort.go('e', testdata.ue4_files, {limit:100})
    fuzziersort.go('mrender.h', testdata.ue4_files, {limit:100})
  }, config)

  bench('huge nomatch', function() {
    fuzziersort.single('xxx', 'noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster')
  }, config)
  bench('huge nomatch !bitflags', function() {
    fuzziersort.single('xxx', 'x noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster noodle monster')
  }, config)
  bench('tricky', function() {
    fuzziersort.single('prrun', 'C:/users/farzher/dropbox/someotherfolder/pocket rumble refactor/Run.bat')
  }, config)
  bench('small', function() {
    fuzziersort.single('al', 'alexstrasa')
  }, config)
  bench('somematch', function() {
    fuzziersort.single('texxx', 'template/index')
  }, config)
}
